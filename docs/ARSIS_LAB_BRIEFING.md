...
| AppData | 使う | 練習・タスク・お知らせ・エキストラ契約 |
| Documents | 使う | 書類台帳（一般／第N回） |
| Concerts / 演奏会 / 演奏会分類 | 使う | 回数の正本（`1` `2` `3`… と日付・会場）。複数タブは全部読む。追記は Concerts |
...
| ブック | 役割 |
|---|---|
| 各回会計 | 決定値と実際。領収書転記、ギャラ、締め |
| ArsisCO 会計 | 見込み・暫定。先の公演まで試算する表 |

- 費目の骨格は詳しい『各回会計』を正にして、見込みへ写す。
- 見込みにはありそうな費目と金額を一通り書く。予算表を実績で上書きしない。
- 使わなかった行は転記時に各回会計で空かゼロのまま残す。公演後に増えた項目は各回会計に足し、次の見込みの骨格にも足す。
- 第2回のように先が長い回は、見込みは全部載せた暫定、確定分だけ各回会計へ写す。
- 団向けの手順は『ArsisCO 運営ガイド』3-1。開発 TODO はここに混ぜない。
