# 書類台帳（Google ドキュメント / Drive）の設定

スプレッドシート連携はそのまま使い、同じサービスアカウントで **Google ドキュメント** と **Drive** も読み書きします。

書類の本文はポータルにコピーしません。台帳（`Documents` シート）に URL・分類・要約を置き、クリックで元ファイルを開きます。

---

## 1. Google Cloud で API を有効にする

団員機能ですでに使っている GCP プロジェクトで、次を有効にします。

1. [Google Cloud Console](https://console.cloud.google.com/) を開く
2. サービスアカウントを作ったプロジェクトを選ぶ
3. 「API とサービス」→「ライブラリ」
4. 次を有効化
   - **Google Drive API**
   - **Google Docs API**
   - Google Sheets API（既存）

---

## 2. Drive フォルダをサービスアカウントに共有する

1. 書類の親フォルダを Google ドライブで開く
2. 「共有」で、サービスアカウントの `client_email`（`xxxx@yyyy.iam.gserviceaccount.com`）を **編集者** として追加
3. フォルダ URL の ID を控える
   - 例: `https://drive.google.com/drive/folders/14I7LIZIiRdObWHwwHRZPW1kwV2Get9G7`
   - ID は `/folders/` の直後

`.env.local` に任意で書きます。未設定なら、マイページのバックアップ用フォルダを使います。

```env
GOOGLE_DRIVE_FOLDER_ID=14I7LIZIiRdObWHwwHRZPW1kwV2Get9G7
```

スプレッドシート自体も、これまで通りサービスアカウントに編集者共有が必要です。アプリ起動時に `Documents` シートが無ければ自動で作ります。

---

## 3. ポータルでの使い方

| 操作 | 内容 |
|---|---|
| URL を登録 | Docs / Sheets / Drive / NotebookLM のリンクを貼る。リンクボタンで題名と要約案を取得 |
| フォルダを辿る | 台帳のフォルダをクリックし、中のファイルを Portal 内で開く。第N回フォルダでは「楽譜」だけ団員掲載できる |
| 団員掲載 | 基本はファイルごとにスイッチ。運営ガイド・アイデアページ・Instagram insight はスイッチ自体を出さない |
| 検索 | フィルターアイコンからタイトル・要約・タグ・演奏会を絞る |
| 演奏会 | 書類ごとに「一般」か「第N回」。回数はシートから読む（下記） |

台帳から外しても、Google 側の原本は消えません。

---

## 4. 回数（一般 / 第N回）

団の回数リストはタブ `Concerts` / `演奏会` / `演奏会分類` です。複数あれば全部読み、番号を一つにします。新しい回の追記は `Concerts`（無ければこの名前で作ります）。書類・団員の参加・次回公演・契約・タスクは同じ番号を使います。

| 対象 | 一般 | 各回 |
|---|---|---|
| 書類 | concertId が空 | `1` → 第1回 |
| 団員 | 参加回なし（プロフィール自体は共通） | concertIds に `1,2` |
| 次回公演 | — | 今日以降のいちばん近い回。日付が無いときは、終えていない回のうち番号が最大（第1回が終わり第2回があるなら第2回）。終えた回は公演日を過去日で入れる |
| 契約・タスク | 回を選ばない | 同じ `1` |

回数の追加はダッシュボード／書類の「第N回を追加」。日付・ホールはダッシュボードの次回公演からその回へ書きます。

分類（会計・規約・公演）は書類の種類で、回数ではありません。

各回の楽譜は、第N回演奏会フォルダの中に題名がちょうど **楽譜** のファイル（複数なら同名フォルダ）を置く。前後に空白を足した「楽譜　」などはずらした名前として扱い、スイッチは出さない。

---

## トラブルシュート

| 現象 | 確認すること |
|---|---|
| 題名が取れない | Drive API・Docs API が有効か。フォルダをサービスアカウントに編集者共有しているか |
| Portal 内で開けない | 上記に加え、フォルダの共有が「閲覧者」になっていないか |
| NotebookLM の中身が取れない | 仕様です。URL と手入力のタイトル・要約だけ登録します |
| シートが見つからない | 同じスプレッドシートに `Documents` が自動作成されます。ブック自体の共有を確認 |
| 演奏会に第1回が出ない | `Concerts`・`演奏会`・`演奏会分類` のどれかに `1` があるか。分類（会計など）の列ではなく回数タブ。書類の `concertId` に `1` があれば回数リストにも出る |
